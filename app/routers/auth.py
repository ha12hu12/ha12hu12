...
def user_login(data: user_login_schema, db: Session = Depends(get_db)):
    ...
    if not the_user:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"there is no user with the email: {data.email}")

    verify_password = utils.verify(plain_password=data.password, hashed_password=the_user.password)    

    if not verify_password:
       raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=f"WRONG PASSWORD, please try again")
